...
// =============================================================================
// get_project_team.php liefert user_id, username, project_role, system_role und
// is_active. Gespeichert wird weiterhin die Benutzer-ID im vorhandenen Feld
// source_contact, damit set_risk.php und bestehende Datensätze kompatibel bleiben.
function populateResponsibleSelect(selectedUserId = '') {
    const select = document.getElementById('risk_responsible');
    if (!select) return;

    const previousValue = String(selectedUserId || select.value || '');

    select.innerHTML =
        '<option value="">-- Niemand zugewiesen --</option>' +
        projectTeam.map(member => {
            const userId = Number(member.user_id);
            const username = esc(member.username || `Benutzer ${userId}`);
            const role = String(member.project_role || member.system_role || '').trim();

            return `<option value="${userId}">${username}${role ? ` (${esc(role)})` : ''}</option>`;
        }).join('');
    ...
    // Eine frühere Zuweisung bleibt nachvollziehbar, auch wenn das Mitglied
    // inzwischen aus dem Projektteam entfernt oder deaktiviert wurde.
    if (!optionExists) {
        select.insertAdjacentHTML(
            'beforeend',
            `<option value="${esc(previousValue)}">Ehemaliges Projektmitglied (ID ${esc(previousValue)})</option>`
        );
    }
...
function getResponsibleName(userId, attributes = {}) {
    const member = projectTeam.find(
        item => Number(item.user_id) === Number(userId)
    );

    if (member) {
        const role = member.project_role || member.system_role || '';
        return role ? `${member.username} (${role})` : member.username;
    }

    if (userId) return `Ehemaliges Projektmitglied (ID ${userId})`;
    return attributes.responsible || '';
}
...
export async function loadRisks() {
    ...
    try {
        // Verantwortliche kommen aus dem aktiven Projektteam. Anforderungen und
        // Risiken werden gleichzeitig geladen, damit die Seite schnell bleibt.
        const [teamResponse, requirementResponse] = await Promise.all([
            fetch(`../api/get_project_team.php?project_id=${encodeURIComponent(currentProjectId)}`),
        ...
        if (!teamResponse.ok || !teamData.success) {
            throw new Error(teamData.error || 'Das Projektteam konnte nicht geladen werden.');
        }

        if (!requirementResponse.ok || !requirementData.success) {
            throw new Error(requirementData.error || 'Risiken konnten nicht geladen werden.');
        }

        projectTeam = (teamData.team || []).filter(
            member => Number(member.is_active ?? 1) === 1
        );

        populateResponsibleSelect();

        loadedRisks = (requirementData.requirements || []).filter(
            item => item.type === 'RISK' && item.review_status !== 'Archiviert'
        );
        ...
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="12" class="p-6 text-center text-red-600">${esc(error.message)}</td></tr>`;
        }
    }
}
