...
function splitAcceptanceCriteria(value) {
    const lines = acPlainLines(value);
    const criteria = [];

    for (const line of lines) {
        /*
         * Normal case: one bullet per actual line.
         * Fallback: Editor.js may flatten several " - item" bullets
         * into one line. Hyphens inside 4-20-mA remain untouched,
         * because a separator needs whitespace on both sides.
         */
...
function stopEditorEnter(event) {
...
function splitEditableAtCaret(element) {
    ...
    holder.appendChild(tailFragment);

    /* Leere Browser-Platzhalter am Ende des alten Eintrags entfernen. */
    while (
        element.lastChild &&
    ...
    add(item, after = null) {
        const row = document.createElement('div'); row.className = 'ds-checklist-row'; const check = document.createElement('input'); check.type = 'checkbox'; check.checked = !!item.checked; const text = document.createElement('div'); text.className = 'ds-checklist-text'; text.contentEditable = 'true'; text.innerHTML = item.text || ''; check.onchange = () => { row.classList.toggle('is-checked', check.checked); changed() }; text.onkeydown = e => {
            if (e.key === 'Enter' && !e.shiftKey) {
                stopEditorEnter(e);
                const trailingHtml = splitEditableAtCaret(text);
                const newRow = this.add(
                    { text: trailingHtml, checked: false },
                    row
                );
                requestAnimationFrame(() => {
                    focusEditableAtEnd(
                        newRow.querySelector('.ds-checklist-text')
                    );
                });
                changed();
                return;
        ...
        const source = Array.isArray(data.items)
            ? data.items.flatMap(item => {
                if (typeof item === 'string') {
                    return splitAcceptanceCriteria(item);
                }

                const parts = splitAcceptanceCriteria(item.text ?? '');

                if (parts.length <= 1) return [item];

                return parts.map((text, index) => ({
                    ...item,
                    id: index === 0 ? item.id : acUuid(),
                    text,
                    revision: index === 0 ? item.revision : 1,
                    status: index === 0 ? item.status : 'open'
                }));
            })
    ...
    static get conversionConfig() {
        return {
            ...
            import: value => ({
                ...
                rule: 'ALL',
                threshold: 1,
                items: splitAcceptanceCriteria(value).map(text => ({
                    id: acUuid(),
                    text,
                    revision: 1,
                    status: 'open'
    ...
    add(item, after = null) {
        ...
        text.onkeydown = e => {
            if (e.key === 'Enter' && !e.shiftKey) {
                ...
                requestAnimationFrame(() => {
                    focusEditableAtEnd(
                        newRow.querySelector('.ds-acceptance-text')
                    );
                });
                changed();
                return;
...
function compareTreeDocuments(left, right) {
    const leftType = String(
        left.requirement_type || ''
    ).toUpperCase();

    const rightType = String(
        right.requirement_type || ''
    ).toUpperCase();

    const leftOrder =
        TREE_TYPE_ORDER[leftType] ?? 999;

    const rightOrder =
        TREE_TYPE_ORDER[rightType] ?? 999;
...
function tree() {
    ...
    /*
     * Schneller Zugriff auf Dokumente über die ID.
     */
    ...
    /*
     * Ein Dokument kann technisch mehrere Parents besitzen.
     * Im Baum verwenden wir den ersten gültigen Parent als
     * primäre Position.
     */
    ...
    for (const document of docs) {
        ...
        const parentId =
            primaryParentByChild.get(documentId);
    ...
    /*
     * Nur Geschwister werden nach Typ und Kennung sortiert.
     * Children bleiben immer direkt unter ihrem Parent.
     */
    rootDocuments.sort(
        compareTreeDocuments
    );
    ...
    /*
     * Bei einer Suche müssen nicht nur die Treffer,
     * sondern auch deren Parent-Ketten sichtbar bleiben.
     */
    ...
    if (searchText !== '') {
        ...
        for (
            ...
            /*
             * Treffer plus alle Parents aufnehmen.
             * guard verhindert Endlosschleifen bei
             * fehlerhaften zyklischen Daten.
             */
            while (
                documentId &&
                guard <= docs.length
            ) {
                visibleDocumentIds.add(
                    documentId
                );

                documentId =
                    primaryParentByChild.get(
                        documentId
                    );
    ...
    function renderDocument(
        ...
        /*
         * Dokumente nicht doppelt anzeigen.
         * Gleichzeitig Zyklen abfangen.
         */
        if (
            renderedDocumentIds.has(
                documentId
            ) ||
            path.has(documentId)
        ) {
        ...
        nextPath.add(documentId);

        const coveragePercent =
            Number(
                document.coverage_percent || 0
            );
        ...
        const tooltip = [
            `${document.requirement_key} · ${document.title}`,
            `Typ: ${document.requirement_type}`,
            `Status: ${document.status}`,
            `Verifikation: ${document.verification_status ||
            'Not Covered'
            }`,
            `Coverage: ${coverageText}`,
            validParentCount > 1
                ? `Parents: ${validParentCount}`
                : ''
        ]
            .filter(Boolean)
            .join('\n');
        ...
        return `
            <button
                type="button"
                class="node ${current?.id === document.id
                ? 'active'
                : ''
            }"
                data-doc="${esc(document.id)}"
                title="${esc(tooltip)}"
            ...
            >
                ...
                ? `
                            <span
                                class="node-multiparent"
                                title="${validParentCount
                } Parents"
                            >
                                ↗${validParentCount}
                            </span>
    ...
    /*
     * Alle regulären Wurzeln rekursiv ausgeben.
     */
    let html = rootDocuments
        .map(document =>
            renderDocument(
                document,
                0,
    ...
    /*
     * Falls fehlerhafte Beziehungen, Zyklen oder
     * unvollständige Parent-Daten vorliegen, bleiben
     * diese Dokumente trotzdem sichtbar.
     */
    const remainingDocuments = [
        ...docs
    ].sort(compareTreeDocuments);

    for (
        const document
        of remainingDocuments
    ) {
        const documentId =
            String(document.id);
...
async function applyDocumentMode() {
    ...
    root.classList.toggle('is-reading', !editMode);

    if (editor?.readOnly && typeof editor.readOnly.toggle === 'function') {
        try {
            await editor.readOnly.toggle(!editMode);
        } catch (error) {
            console.warn('Editor.js readOnly konnte nicht umgeschaltet werden:', error);
        }
    }
    ...
    document.querySelectorAll('#requirementsEditor [contenteditable]').forEach(element => {
        element.contentEditable = editMode ? 'true' : 'false';
        element.tabIndex = editMode ? 0 : -1;
    });

    document.querySelectorAll('#requirementsEditor input,#requirementsEditor select,#requirementsEditor textarea').forEach(element => {
        element.disabled = !editMode;
        element.tabIndex = editMode ? 0 : -1;
    });

    document.querySelectorAll('#v4Panel input,#v4Panel select,#v4Panel textarea').forEach(element => {
        element.disabled = !editMode;
        element.tabIndex = editMode ? 0 : -1;
    });
...
async function enterEditMode() {
    ...
    editMode = true;
    dirty = false;
    await applyDocumentMode();
    setTimeout(() => { applyDocumentMode(); }, 80);
}
...
async function deleteCurrentDocument() {
    ...
    if (typedId.trim() !== confirmationId) {
        window.alert('Die eingegebene Dokument-ID stimmt nicht überein. Das Dokument wurde nicht gelöscht.');
        return;
    }

    if (dirty && !window.confirm('Ungespeicherte Änderungen werden ebenfalls verworfen. Fortfahren?')) {
        return;
    }

    state('Löschen ...');

    await api(`${A}delete_document.php`, {
        method: 'POST',
        body: JSON.stringify({
            id: current.id,
            confirmation_key: confirmationId
        })
    });
    ...
    if (editor) {
        try {
            ...
            console.warn('Editor konnte nach dem Löschen nicht vollständig geschlossen werden:', error);
        }
    }
    ...
    $('v4Work')?.classList.add('hidden');
    $('v4Empty')?.classList.remove('hidden');
    state('Bereit');
