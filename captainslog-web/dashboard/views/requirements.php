...
<div class="flex flex-col h-full">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-blue-900">Ziele & Anforderungen</h2>
        <button id="new" class="rounded bg-blue-900 px-4 py-2 font-semibold text-white shadow hover:bg-blue-800 transition">
            + Neuer Eintrag
        </button>
    </div>
    <div class="grid gap-5 lg:grid-cols-[350px_1fr] flex-1 min-h-0">
...
<div id="reqModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/70 p-4 sm:p-6 backdrop-blur-sm">
    <form id="reqForm" class="w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col space-y-4 rounded-xl bg-white p-6 sm:p-8 shadow-2xl">
        ...
        <div>
            ...
            <select id="type" class="w-full rounded border p-2 text-sm bg-slate-50 font-medium focus:border-blue-500 outline-none">
                <optgroup label="Ziele & Strategie">
                    <option value="GOAL">Ziel (Stakeholder Goal)</option>
                </optgroup>
                <optgroup label="Regularien & Risiken">
                    <option value="NORM">Norm / Standard (ISO, IEC, CRA)</option>
        ...
        <label class="block text-sm font-semibold text-slate-700">Titel / Name
            <input id="title" required class="mt-1 w-full rounded border p-2 font-normal focus:border-blue-500 outline-none">
        </label>
        
        <label class="block text-sm font-semibold text-slate-700">Beschreibung
            <textarea id="text" required rows="3" class="mt-1 w-full rounded border p-2 font-normal focus:border-blue-500 outline-none"></textarea>
        </label>
        
        <label class="block text-sm font-semibold text-slate-700">Begründung (Rationale)
            <textarea id="rationale" rows="2" class="mt-1 w-full rounded border p-2 font-normal focus:border-blue-500 outline-none"></textarea>
        </label>

        <!-- Dynamische Attribute -->
        <div id="dynamicAttributes" class="hidden rounded-lg bg-indigo-50 p-4 border border-indigo-100 shadow-inner mt-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            <label class="block text-sm font-semibold text-slate-700">Aufwandschätzung
                <input id="effort" type="text" placeholder="z.B. 3 Tage / 5 Story Points" class="mt-1 w-full rounded border p-2 font-normal outline-none">
            </label>
        </div>

        <label class="block text-sm font-semibold text-slate-700 mt-2">Akzeptanzkriterien (Ein Kriterium pro Zeile)
            <textarea id="acceptance_criteria" rows="3" placeholder="- Kriterium 1&#10;- Kriterium 2" class="mt-1 w-full rounded border p-2 font-normal outline-none"></textarea>
        </label>
        ...
        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-200 pt-5 mt-2">
            <div>
                ...
                <input type="text" id="parentSearch" placeholder="Suchen..." class="w-full text-xs rounded border p-1.5 mb-2 bg-slate-50 outline-none" oninput="window.filterCheckboxes('parentSearch', 'parentsCheckboxList')">
                <div id="parentsCheckboxList" class="h-40 overflow-y-auto rounded border bg-white p-2 space-y-1 text-xs shadow-inner"></div>
            </div>
            <div>
                <label class="text-sm font-semibold block mb-1 text-slate-700">Children (Wird erfüllt durch)</label>
                <input type="text" id="childSearch" placeholder="Suchen..." class="w-full text-xs rounded border p-1.5 mb-2 bg-slate-50 outline-none" oninput="window.filterCheckboxes('childSearch', 'childrenCheckboxList')">
                <div id="childrenCheckboxList" class="h-40 overflow-y-auto rounded border bg-white p-2 space-y-1 text-xs shadow-inner"></div>
            </div>
        </div>

        <div class="flex justify-end gap-3 mt-6 border-t pt-4">
            <button type="button" onclick="document.getElementById('reqModal').classList.add('hidden')" class="rounded border px-4 py-2 hover:bg-slate-50 font-medium transition">Abbrechen</button>
            <button type="submit" class="rounded bg-blue-900 px-5 py-2 font-medium text-white shadow hover:bg-blue-800 transition">Speichern</button>
        </div>
    </form>
</div>

<!-- Modal für Akzeptanzkriterien-Prüfung -->
<div id="verifyModal" class="hidden fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/70 p-4 backdrop-blur-sm">
    <form id="verifyForm" class="w-full max-w-md space-y-4 rounded-xl bg-white p-6 shadow-2xl">
        ...
        <label class="block text-sm font-semibold text-slate-700 mt-4">Notiz / Link zum Testprotokoll
            <textarea id="verify_note" required rows="3" placeholder="z.B. Test T-045 erfolgreich durchgeführt. Link: \\Server\..." class="mt-1 w-full rounded border p-2 font-normal focus:border-blue-500 outline-none"></textarea>
        </label>

        <div class="flex justify-end gap-3 mt-6 border-t pt-4">
            <button type="button" onclick="document.getElementById('verifyModal').classList.add('hidden')" class="rounded border px-4 py-2 hover:bg-slate-50 font-medium transition">Abbrechen</button>
            <button type="submit" class="rounded bg-emerald-600 px-5 py-2 font-medium text-white shadow hover:bg-emerald-500 transition">Als 'Geprüft' markieren</button>
        </div>
    </form>
</div>
