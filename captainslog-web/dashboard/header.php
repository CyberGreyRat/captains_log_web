...
<body class="bg-slate-50 text-slate-800 flex flex-col h-screen overflow-hidden">
    ...
    <header class="border-t-4 border-amber-400 bg-[#0d3158] text-white shrink-0">
        <!-- Volle Breite (w-full), reduziertes Padding, kompaktere Höhe -->
        <div class="flex h-16 w-full items-center justify-between px-4">
            <div class="flex items-center gap-4">
                <a href="index.php" class="flex items-center gap-4 hover:opacity-80 transition cursor-pointer">
                    <img src="css/logo.png" class="h-12 w-auto" alt="Captain's Log Logo">
                    <div>
                        <p class="text-[10px] uppercase tracking-[.2em] text-blue-200">Requirements • Risk • Verification
                        </p>
                        <h1 class="text-xl font-bold">Captain's Log</h1>
                    </div>
                </a>
            </div>
            <div class="flex items-center gap-3">
                ...
                <div class="user-info text-xs text-blue-100 hidden md:block">
                    Eingeloggt als: <strong class="text-white"><?php echo htmlspecialchars($username); ?></strong>
                    @ <?php echo htmlspecialchars($hostname); ?>
                </div>

                <select id="projectSelect" class="text-slate-800 px-2 py-1.5 text-sm font-bold outline-none rounded">
                ...
                <button type="button" onclick="window.openNewProjectModal()"
                    class="rounded border border-white px-3 py-1.5 text-xs font-bold transition hover:bg-white hover:text-[#0d3158]">
                    Neues Projekt
                </button>
